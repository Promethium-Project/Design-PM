...
**Dépendances :** 
- [[File = Folder]]
 - [[Système de git pour les dossiers et documents]], 
-  [[Création de decks Anki]] 
-  [[Gestion des exercices d'oraux]] 
 - [[Analytics autour d'un dossier]]
 - [[Système de Templates]]
- [[Supérieur/Design PM/Features/Gestion de l'OCR]]
- [[Coopération sur un dossier]]

# Feature Spec — Anchor System & Mapping

**Statut :** Proposition **Auteur :** Claude **Date :** 2026-02-25 **Dépendances :** Nœud Composite & DisplayTemplate, Système git, Création de decks Anki, Gestion des questions, Gestion de l'OCR, Coopération sur un dossier, Analytics autour d'un dossier
...
Ici c'est plus tôt soit il y a un display sur le document. 
Soit il y en a pas et juste on auto-scroll quand l'autre document est mis en sidepane
...
Cett idée d'afficher les variables, c'est juste une anchor intra document avec un display overlay ! [[variables]]
...
**-> là ça fait totalement sens cette idée de marge !**
...
**Mode split-screen** (mode travail) Deux panneaux côte à côte. Le panneau gauche affiche la source. Le panneau droit affiche un document de notes ou tout autre Node cible. Les anchors reliant ces deux documents sont visualisés dans les deux panneaux simultanément.
...
**Mode document avec overlays** (mode lecture) La source est affichée seule en plein écran. Les anchors dont le template est `overlay` ou `icon` sont actifs et réagissent au survol/clic. Les anchors `panel` sont désactivés ou réduits à une icône dans la marge.

**Mode marginalia** (mode révision) La source occupe ~70% de l'écran. Les notes ancrées s'affichent dans la marge droite (~30%), alignées verticalement avec leur position source. Chaque note est compacte ; au clic, elle s'expand en overlay.
...
## 9. Remarques & Questions ouvertes

**1** — _Oui c'est carrément bidirectionnel, on mets en bijection les deux documents _

**2 — Gestion des anchors sur PDF non-extractable** Certains PDFs (scans non-OCR'd, PDFs protégés) ne permettent pas la sélection de texte. L'anchor doit alors se baser uniquement sur les coordonnées image. Si le PDF est re-uploadé en version OCR'd ultérieurement, les anchors image-seulement peuvent-ils être automatiquement réconciliés avec la couche texte ? -> il restent en mode coordonnées... et il faut les redéfinir pour passer en mode texte

**3 — Limite du nombre de layers simultanés** Avec des anchors personnels (notes, Anki, signets), des anchors de guilde, des anchors forum, et des anchors de définition, un document source très annoté peut devenir visuellement surchargé. Une règle de priorité d'affichage ou une limite de densité visuelle doit être définie. Proposition : quand plusieurs anchors se chevauchent sur la source, les fusionner en un seul marqueur "cluster" avec un badge de compte, qui s'expand au survol. 
**C'EST UNE PTN D'IDEE LA GUILDE FAIT DES CLUSTERS**


**4 — Export des anchors** Lors de l'export d'un document source en PDF (via le DisplayTemplate export), les anchors sont-ils inclus ? Si oui, sous quelle forme (annotations PDF standard, notes de bas de page, QR codes vers les Nodes cibles) ? La réponse détermine si les documents exportés de Prométhium restent "vivants" ou deviennent des copies mortes.

**Je dirais que l'on peut exporter un document avec un .promethium lorsqu'il est dans un dossier, qui pointe vers soit les documents du même dossier, soit des ids dans la base de donnée de Prométhium, l'idée c'est de conserver au maximum les fonctionnalités qui sont en local, mais de ne rien faire sortir de notre db**